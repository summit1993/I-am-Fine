...
def model_process(model, loaders, optimizer, config_info, log_file_name_prefix, model_save_dir):
    ...
    local_loss = partial(Poker_loss, hierarchy=config_info['hierarchy'], loss_fn=config_info['loss_fn'])
    ...
    for epoch in range(epoch_num):
        ...
        for step, data in enumerate(train_loader, 0):
            ...
            loss_local = local_loss(outputs[1], labels, device)
            loss_global = global_loss(outputs[0], labels)
